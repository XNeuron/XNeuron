...
void XNeuronTestTest::XOR()
{
	QList<QList<bool>> i;
	QList<bool> o;

	XNXOR.mNextNeuron.append(new GradientTraining());
	XNXOR.mNextNeuron.append(new GradientTraining());
	XNXOR.mFirstNeuron.append(XNXOR.mNextNeuron[0]);
	XNXOR.mFirstNeuron.append(XNXOR.mNextNeuron[1]);
	XNXOR.mLastNeuron.append(&XNXOR);
	XNXOR.mFirstNeuron[0]->mLastNeuron = XNXOR.mLastNeuron;
	XNXOR.mFirstNeuron[1]->mLastNeuron = XNXOR.mLastNeuron;

	XNXOR.mFirstNeuron[0]->mNextNeuron = XNXOR.mLastNeuron;
	XNXOR.mFirstNeuron[1]->mNextNeuron = XNXOR.mLastNeuron;

	XNXOR.mFirstNeuron[0]->setWeight(QList<double>() << 0.5 << 0.5);
	XNXOR.mFirstNeuron[1]->setWeight(QList<double>() << 1 << 1);

	XNXOR.setWeight(QList<double>() << 1 << -1);
	...
	for (QList<bool>& elem : i)
	{
		XNXOR.mFirstNeuron[0]->setInput(elem);
		XNXOR.mFirstNeuron[1]->setInput(elem);
		XNXOR.setInput(XNXOR.mFirstNeuron[0]->outputBinary(), XNXOR.mFirstNeuron[1]->outputBinary());
		...
		QStringList str2;
		str2.append(str.join("x"));
		str2.append(QString().number(XNXOR.output()));
		str2.append(QString().number(XNXOR.bias()));
		str2.append(XNXOR.outputBinary() ? "1" : "0");
		str2.append(o[index] ? "1" : "0");
	//	QVERIFY2(XNXOR.outputBinary() == o[index], str2.join("=").toStdString().c_str());
	}
}
